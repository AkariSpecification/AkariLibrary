...
def rsaConnection(host, username, key_filename, port=22):
    return fabric.Connection(host, user=username, connect_kwargs={"key_filename": key_filename}, port=port)

def passwordConnection(host, username, password, port=22):
    return fabric.Connection(host, user=username, connect_kwargs={"password": password}, port=port)
